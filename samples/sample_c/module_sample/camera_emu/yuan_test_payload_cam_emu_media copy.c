...
//     // Create a temporary file to save data received from UDP
//     char tempFilePath[DJI_FILE_PATH_SIZE_MAX];
//     snprintf(tempFilePath, DJI_FILE_PATH_SIZE_MAX, "%s/temp.%s", directory, outFormat);
//     FILE *tempFile = fopen(tempFilePath, "wb");
//     if (tempFile == NULL ) {
//         USER_LOG_ERROR("create temp file fail.");
//         returnCode = DJI_ERROR_SYSTEM_MODULE_CODE_SYSTEM_ERROR;
//     }
...
//     char* fixed_ip = "127.0.0.1";
//     int fixed_port = 8888;

//     memset(&server_addr, 0, sizeof(server_addr));
//     server_addr.sin_family = AF_INET;
//     if ( inet_pton(AF_INET, fixed_ip, &server_addr.sin_addr) <= 0 ) {
//         USER_LOG_ERROR("Invalid IP address.");
//         fclose(tempFile);
//         returnCode = DJI_ERROR_SYSTEM_MODULE_CODE_SYSTEM_ERROR;
//     }

//     if (bind(sockfd, (struct sockaddr *)&server_addr, sizeof(server_addr)) < 0) {
//         USER_LOG_ERROR("bind socket fail.");
//         close(sockfd);
//         fclose(tempFile);
//     } */

//     // Receive data and write it to a temporary file
//     char buffer[BUF_SIZE];
//     struct sockaddr_in server_addr;
//     socklen_t addr_len = sizeof(server_addr);
    
//     int num_bytes = recvfrom(sockfd, buffer, MAX_BUFFER_SIZE, 0, (struct sockaddr *)&server_addr, &addr_len);
//     if (num_bytes < 0) {
//         USER_LOG_ERROR("Receive data fail.");
//         fclose(tempFile);
//         return DJI_ERROR_SYSTEM_MODULE_CODE_SYSTEM_ERROR;
//     }
//     fwrite(buffer, 1, num_bytes, tempFile);

//     fclose(tempFile);

//     // create ffmpeg command !!!
//     snprintf(ffmpegCmdStr, FFMPEG_CMD_BUF_SIZE,
//              "echo \"y\" | ffmpeg -i \"%s\" -codec copy -f \"%s\" \"%s\" 1>/dev/null 2>&1", tempFilePath,
//              outFormat, outPath);
...
static T_DjiReturnCode DjiPlayback_GetFrameRateOfVideoFile(const char *path, float *frameRate)
{
    ...
    snprintf(ffmpegCmdStr, FFMPEG_CMD_BUF_SIZE, "ffprobe -show_streams udp://127.0.0.1:8888 2>/dev/null");
    fpCommand = popen(ffmpegCmdStr, "r");
    if (fpCommand == NULL) {
...
// 分析 UDP 数据包，提取每个 NALU 单元
// int extractNALU(uint8_t *data, size_t dataSize) {
//     size_t startPos = 0;
//     while (startPos < dataSize) {
//         // 查找NALU的起始码
//         size_t naluStart = -1;
//         for (size_t i = startPos; i < dataSize - NALU_START_CODE_LEN + 1; i++) {
//             if (memcmp(data + i, NALU_START_CODE, NALU_START_CODE_LEN) == 0) {
//                 naluStart = i;
//                 break;
//             }
//         }

//         if (naluStart == (size_t)-1) {
//             break;  // 没有更多的NALU
//         }

//         // 查找下一个NALU的起始码
//         size_t naluEnd = naluStart + NALU_START_CODE_LEN;
//         while (naluEnd < dataSize - NALU_START_CODE_LEN + 1 && 
//                memcmp(data + naluEnd, NALU_START_CODE, NALU_START_CODE_LEN) != 0) {
//             naluEnd++;
//         }

//         // 如果找到下一个NALU的起始码，则提取当前NALU
//         size_t naluSize = naluEnd - naluStart;
//         if (naluSize > 0) {
//             uint8_t *naluData = (uint8_t *)malloc(naluSize);
//             if (naluData != NULL) {
//                 memcpy(naluData, data + naluStart, naluSize);
//                 // 处理当前NALU数据（例如发送到DjiPayloadCamera_SendVideoStream）
//                 DjiPayloadCamera_SendVideoStream(naluData, naluSize);
//                 free(naluData);
//             }
//         }
...
int extractNALU(uint8_t *data, size_t dataSize) {
    ...
    bool IsFoundOneNALU = 0;
    int IsSPSframe = 0;

    while (startPos < frameBufferLen) {
        // search for a NALU Start Code
        size_t naluStart = -1;
        ...
        //     }
        // }
        naluStart = FindStartCodePos(frameBuffer, startPos, frameBufferLen);      
        if ( !naluStart ) { // if there even no one Start Code, then break the while loop
            USER_LOG_ERROR("no one NALU found in one RevData");
        ...
        size_t naluEnd = naluStart + NALU_START_CODE_LEN;
        naluEnd = FindStartCodePos(frameBuffer, naluEnd, frameBufferLen);
        if ( !naluEnd ) { // can't find next NALU. Save data in frambuffer. and find in second data
            memmove(frameBuffer, frameBuffer + naluStart, frameBufferLen - naluStart); // move NALU fragment front
        ...
        // if this NALU is SPS. (we send [SPS PPS IDR] as one NALU)
        if ( memcmp(frameBuffer + startPos + NALU_START_CODE_LEN , &SPS_CODE, 1) == 0 ) {
            IsSPSframe = 1;
            USER_LOG_ERROR("SPS NALU founded");
            DJI_AUD_Info_Add(frameBuffer, &naluEnd, &frameBufferLen); // add DJI AUDinfo in SPS
            naluEnd = naluEnd + NALU_START_CODE_LEN; // naluEnd = PPSstartpos
            
            naluEnd = FindStartCodePos(frameBuffer, naluEnd, frameBufferLen); // if finded. naluEnd = IDRstartpos
            if (naluEnd) {
                USER_LOG_ERROR("IDR NALU cant found");
                return -1;
            }
            DJI_AUD_Info_Add(frameBuffer, &naluEnd, &frameBufferLen); // add DJI AUDinfo in IDR NALU
            printf("IDR nalu type : 0x%02X\n", frameBuffer[naluEnd+NALU_START_CODE_LEN]);
            naluEnd = naluEnd + NALU_START_CODE_LEN;    
            naluEnd = FindStartCodePos(frameBuffer, naluEnd, frameBufferLen); // find IDR next start code
            if (naluEnd) {
                USER_LOG_ERROR("IDR'next NALU cant found");
                return -1;
            }

        }

        // if have find two Start Code or find [SPS,PPS,IDR] ，extract one NALU，then send
        size_t naluSize = naluEnd - naluStart;
        if (naluSize > 0) {
            // 为NALU数据加上音频数据部分（frame audio info）
            ...
            if (naluData != NULL) {
                ...
                while (lengthToSend > 0) {
                    ...
                    if (returnCode != DJI_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
                        ...
                        USER_LOG_INFO("send video stream %d byte.", sendSize);
                        if ( IsSPSframe==1 ) { 
                            printf("Sent %zu bytes.", sendSize);
                            for (size_t i = 0; i < 50; i++) {
                                printf("0x%02x ", naluData[offset + i]);
                            }
